...
class LoggedMongoDBSessionStorage {
  ...
  // ─── FIX 1: Never overwrite a good session with a tokenless one ───────────
  async storeSession(session) {
    console.log("[SessionStorage] storeSession() called");
    console.log("[SessionStorage] Session shop:", session?.shop);
    console.log("[SessionStorage] Session id:", session?.id);
    ...
    if (!session?.accessToken) {
      console.log("[SessionStorage] ⚠️  BLOCKED storeSession — no accessToken, skipping to protect existing session");
      return true;
    }
  ...
  // ─── FIX 2: Normalize ids to array — Shopify lib sometimes passes a string ─
  async deleteSessions(ids) {
    const idsArray = Array.isArray(ids) ? ids : [ids];
    console.log("[SessionStorage] deleteSessions() called with ids:", idsArray);
    try {
      const result = await this._inner.deleteSessions(idsArray);
      console.log("[SessionStorage] deleteSessions() ✅ result:", result);
      return result;
    } catch (err) {
      console.log("[SessionStorage] deleteSessions() ❌ ERROR:", err.message);
...
const shopify = shopifyApp({
  ...
  distribution: AppDistribution.AppStore,
  future: {
    // expiringOfflineAccessTokens: true,  ← keep disabled, was corrupting sessions
    unstable_newEmbeddedAuthStrategy: true,
  },
  ...(process.env.SHOP_CUSTOM_DOMAIN
    ? { customShopDomains: [process.env.SHOP_CUSTOM_DOMAIN] }
